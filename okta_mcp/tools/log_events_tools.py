...
def register_log_events_tools(server: FastMCP, okta_client: OktaMcpClient):
    ...
    async def get_logs(
        ...
        try:
            ...
            if ctx:
                ctx.info(f"Getting logs with parameters: since={since}, until={until}, filter={filter_string}, q={q}")
            ...
            if err:
                ...
                if ctx:
                    ctx.error(f"Error retrieving log events: {err}")
            ...
            while resp and hasattr(resp, 'has_next') and resp.has_next():
                ...
                try:
                    ...
                    if next_err:
                        ...
                        if ctx:
                            ctx.error(f"Error during pagination: {next_err}")
                        break
                    ...
                    if ctx:
                        ctx.error(f"Exception during pagination: {str(e)}")
                    break
            
            if ctx:
                ctx.info(f"Retrieved {len(all_log_events)} log events across {page_count} pages")
